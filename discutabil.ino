...
long duration;
long distance;  // Folosită global

// Flag pentru modul autonom (obstacole)
bool autonomousMode = false;
bool autonomousFireMode = false;
...
void toggleAutonomousFireMode();
void stopAutonomousMode();
void stopAutonomousFireMode();
void obstacleAvoidance();
void obstacleAvoidanceWithServo();
void checkBackward();
...
void handleRoot() {
  ...
  String html = R"rawliteral(
    ...
    <body>
        ...
        <button onclick="sendCommand('toggleAutonomousFireMode')">Toggle Autonomous FireMode</button>
        <button onclick="sendCommand('stopAutonomousFIreMode')">Stop Autonomous Fire Mode</button>
...
// ==================== Măsurare Distanță (Noua implementare) ====================
void measureDistance() {
  // Măsurăm de mai multe ori și facem medie, păstrând structura existentă
  ...
  for (int i = 0; i < numSamples; i++) {
    ...
    // Măsurăm durata impulsului returnat de senzor
    long localDuration = pulseIn(echoPIN, HIGH);

    // Calculăm distanța în centimetri
    long dist = localDuration / 58.2;

    // Acceptăm doar valori plauzibile (e.g., < 400 cm)
    if (dist > 0 && dist < 400) {
      totalDist += dist;
      validSamples++;
    }
    // O mică întârziere între măsurători
    delay(30);
  }

  // Calculăm media dacă avem eșantioane valide
  if (validSamples > 0) {
    distance = totalDist / validSamples;
  } else {
    // Dacă nu e nicio măsură validă, setăm la un fallback mare
    distance = 999;
  }

  // Trimitem valoarea calculată către client
...
void toggleAutonomousFireMode() {
  autonomousFireMode = !autonomousFireMode;
  server.send(200, "text/plain", autonomousFireMode ? "Mod Autonom ACTIVAT" : "Mod Autonom DEZACTIVAT");
}

void stopAutonomousFireMode() {
  autonomousFireMode = false;
  server.send(200, "text/plain", "Mod Autonom Oprit");
}
...
void obstacleAvoidance() {
  switch (currentState) {
    case LOOK_FORWARD:
      setServo2To90(); // Setare servo la poziția frontală
      delay(2000);
      measureDistance();
      if (distance > 40) {
        currentState = MOVE_FORWARD; // Dacă distanța din față este OK, mergi înainte
      } else {
        currentState = LOOK_LEFT; // Dacă nu, verifică în stânga
      }
      break;

    case LOOK_LEFT:
      rotateServo2Clockwise(); // Mută servo la stânga
      delay(2000);
      measureDistance();
      if (distance > 40) {
        currentState = TURN_LEFT; // Dacă distanța din stânga este OK, întoarce la stânga
      } else {
        currentState = LOOK_RIGHT; // Dacă nu, verifică în dreapta
      }
      break;

    case LOOK_RIGHT:
      rotateServo2CounterClockwise(); // Mută servo la dreapta
      delay(2000);
      measureDistance();
      if (distance > 40) {
        currentState = TURN_RIGHT; // Dacă distanța din dreapta este OK, întoarce la dreapta
      } else {
        currentState = MOVE_BACKWARD; // Dacă nicio direcție nu este viabilă, mergi înapoi
      }
      break;

    case MOVE_FORWARD:
      moveForwardBoth(); // Mergi înainte
      delay(2500);
      stopBothMotors(); // Oprește motoarele
      currentState = LOOK_FORWARD; // Revino la verificarea frontală
      break;

    case TURN_LEFT:
      turnLeft(); // Întoarce la stânga
      delay(2000);
      stopBothMotors();
      currentState = LOOK_FORWARD; // Revino la verificarea frontală
      break;
    ...
    case STOP_BACKWARD:
      stopBothMotors(); // Oprește robotul
      break;
  ...
  switch (currentState) {
    case LOOK_FORWARD:
      ...
      if (distance > 40) {
        ...
        rotateServo2CounterClockwise(); // Activează servo când obstacolul este detectat
        delay(1000); // Lasă timp pentru rotație
        currentState = LOOK_LEFT; // Dacă nu, verifică în stânga
      }
      break;
...
void updatePID() {
  ...
  // **Trimitem toate valorile PID la Serial pentru plotare**
  Serial.print("error: ");
  Serial.print(error);
...
void sendPIDDataJSON() {
  ...
  // **Send PID output to Serial only if PID Mode is active**
  if (pidMode) {
    Serial.println(outputPID);
  }
...
void setup() {
  ...
  server.on("/toggleAutonomousFireMode", toggleAutonomousFireMode);
  server.on("/stopAutonomousFireMode", stopAutonomousFireMode);
...
void loop() {
  ...
  if (autonomousFireMode) {
    obstacleAvoidanceWithServo();
  }
